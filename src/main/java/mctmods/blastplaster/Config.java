...
public class Config {
  ...
  static {
    ...
    BLOCK_CONVERSIONS = builder.comment("Convert blocks caught in an explosion instead of putting them back or dropping them as they were, so a structure degrades a step per blast.",
                    ...
                    "Source is a block id (minecraft:stone) or a block tag with a leading # (#minecraft:logs). The ore dictionary is the c: tag namespace, so #c:ores/iron is an oredict rule.",
