...
public final class BlockConversions {
  ...
  @Nullable private static BlockState convert(ServerLevel level, RuleSet ruleSet, BlockState state) {
    ...
    for (Rule rule : ruleSet.rules()) {
      ...
      if (rule.chance() < 1.0f && level.random.nextFloat() >= rule.chance()) { return null; }
  ...
  @Nullable private static Rule parseRule(String line) {
    ...
    if (source.startsWith("#")) {
      ResourceLocation tagLoc = ResourceLocation.tryParse(source.substring(1));
      if (tagLoc == null) {
    ...
    ResourceLocation blockLoc = ResourceLocation.tryParse(source);
    Block block = blockLoc == null ? null : ForgeRegistries.BLOCKS.getValue(blockLoc);
  ...
  @Nullable private static Block resultBlock(String text, String line) {
    ...
    Block block = loc == null ? null : ForgeRegistries.BLOCKS.getValue(loc);
    if (block == null || block == Blocks.AIR) {
